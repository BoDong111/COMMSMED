...
#mydf=pandas.read_csv('full_data_12_19_deng_new.csv')
mydf=pandas.read_csv('data_0930_chkk_noncluster.csv')
#mydf=pandas.read_csv('covid_final_data1104.csv')
mydf=mydf.fillna(0)
#####mydf=mydf.sample(n=1000)
##df1 = mydf.iloc[0:10000,1:-1]
##y = mydf.iloc[0:10000,-1]
df1 = mydf.iloc[:,1:-1]
y = mydf.iloc[:,-1]
#print('df')
#print(df)
...
print('testing')
print(shap_values[0])
#newlist=map(abs, shap_values[0])
avg = np.mean(shap_values[0], axis=0)
print('avg')
print(avg)
newlist=list(map(abs, avg))
socio=sum(newlist[0:7])
climate=sum(newlist[7:14])
print('socio')
print(socio)
print('climate')
print(climate)
print(socio/(socio+climate))
print(climate/(climate+socio))
print(socio/(socio+climate)-climate/(climate+socio))
#avg1 = np.mean(shap_values, axis=0)
#print(avg1)
...
data = pandas.DataFrame(X_train, columns=['Illiteracy','Population without health services',
                                           ...
                                           'Houses without sewage','Houses without electricity',
                                           'Mean max temp','Mean min temp','Mean temp',
                                           'Mean mm/day','Max mm/day','Min mm/day','Altitude'])
#print('data')
#print(data)
columns=['Illiteracy','Population without health services',
                                           'Houses with floor of dirt','Houses without toilet','Houses without water pipelines',
                                           'Houses without sewage','Houses without electricity',
                                           'Mean max temp','Mean min temp','Mean temp',
                                           'Mean mm/day','Max mm/day','Min mm/day','Altitude']
#columns=['FIPS', 'Prevalence of obesity', 'Asthma', 'Hypertension', 'Diabetes','HOUSEHOLDS','FAMILIES', 'AVE_FAM_SZ', 'HSE_UNITS','Temp','Mean T', 'Min T', 'Max T', 'Total P', 'RH']
shap.summary_plot(shap_values, data, show=False, feature_names=columns)
#newpd=pandas.DataFrame(shap_values)
#plt.barh(align='center')
plt.savefig('summary_fig.png',bbox_inches='tight', pad_inches=0.1)
